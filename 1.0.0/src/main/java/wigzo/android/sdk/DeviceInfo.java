...
class DeviceInfo {
    ...
    static String getResolution(final Context context) {
        ...
        try {
            ...
            if (Wigzo.sharedInstance().isLoggingEnabled()) {
                Log.i(Wigzo.TAG, "Device resolution cannot be determined");
            }
        }
        return resolution;
    }
    ...
    static String getCarrier(final Context context) {
        ...
        if (carrier == null || carrier.length() == 0) {
            carrier = "";
            if (Wigzo.sharedInstance().isLoggingEnabled()) {
                Log.i(Wigzo.TAG, "No carrier found");
            }
        }
        return carrier;
    }
    ...
    static String getAppVersion(final Context context) {
        String result = Wigzo.DEFAULT_APP_VERSION;
        try {
            result = context.getPackageManager().getPackageInfo(context.getPackageName(), 0).versionName;
        }
        catch (PackageManager.NameNotFoundException e) {
            if (Wigzo.sharedInstance().isLoggingEnabled()) {
                Log.i(Wigzo.TAG, "No app version found");
            }
        }
        return result;
    }
    ...
    static String getStore(final Context context) {
        ...
        if(android.os.Build.VERSION.SDK_INT >= 3 ) {
            try {
                ...
                if (Wigzo.sharedInstance().isLoggingEnabled()) {
                    Log.i(Wigzo.TAG, "Can't get Installer package");
                }
            }
            if (result == null || result.length() == 0) {
                result = "";
                if (Wigzo.sharedInstance().isLoggingEnabled()) {
                    Log.i(Wigzo.TAG, "No store found");
                }
            }
        }
        return result;
